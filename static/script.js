...
function updateMonthSelector(transactionsByMonth) {
  const pieChartCol = document.querySelector(".col-md-6:has(#pieChart)");
  if (pieChartCol) {
    ...
    if (!monthSelect) {
      ...
      select.onchange = (e) => {
        ...
        loadPieChartData(currentMonth); // Only update pie chart for the selected month
      };
      if (Object.keys(transactionsByMonth).length > 0) {
        // Filter to include only January, February, and March 2025
        const validMonths = ["2025-02", "2025-03", "2025-04"];
        ...
        console.log(
          "Inserting monthSelect dropdown into pie chart area with months:",
          validMonths
        );
        pieChartCol.insertBefore(
          select,
          pieChartCol.querySelector("canvas#pieChart")
        );
      } else {
    ...
    console.error(
      "❌ Pie chart column (.col-md-6:has(#pieChart)) not found in DOM."
    );
  }
}
